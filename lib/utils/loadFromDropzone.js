...
var doProcessFileToActive = (dropboxConnection) => {
	...
	return (file, callback) => {
		...
		var tasks = [
			...
			(callback) => {
				generateInvFromFile(file).save()
				.then((invoice) => {
					Group.findOne({ toKey: true }, (err, keyGroup) => {
						keyGroup._queue.push(invoice._id);

						keyGroup.save().then((doc) => callback(null))
					});
				});
			},

			(callback) => { dropboxConnection.moveFile(oldPath, newPath, callback) },
			// #1: make a new invoice object in the database to correspond
