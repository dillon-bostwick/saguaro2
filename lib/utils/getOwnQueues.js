...
const loadFromDropzone = require('./loadFromDropzone.js')
...
var getOwnQueues = (userId, callback) => {
	User.findById(userId, (err, user) => {
		...
		user.deepPopulate('_groups._queue personalQueue._invoice', (err, populUser) => {
			if (err) return callback(err, null);

			loadFromDropzone(user.currentToken, (err) => {
				if (err) return callback(err, null)

				return callback(null, parseUserForQueues(populUser));
			})
		});
	});
}
