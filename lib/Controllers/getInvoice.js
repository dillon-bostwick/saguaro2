...
var getInvoice = (req, res, next) => {
	...
	Invoice.findById(invoiceId, (err, invoice) => {
		...
		if (invoice.dropboxId) {
			dropboxConnection.getLink(invoice.dropboxId, (err, dbRes) => {
				if (err) return next(err); // TODO: the request shouldn't completely fail if the link wasn't found

				if (dbRes) {
					resObj.fileLink = dbRes.link;
					resObj.fileName = dbRes.metadata.name;
				}
			...
			//dropboxId does not exist - do not find link
			findLocation(user, invoice, resObj, (err, resObj) => {
				if (err) return next(err);
