...
var getInvoice = (req, res, next) => {
	...
	Invoice.findById(invoiceId, (err, invoice) => {
		if (err) {
			...
			// Now try the archives to see if there's a match there...
			return Archive.findById(invoiceId, (err, archive) => {
				if (err) return next(err);
				if (!archive) return res.sendStatus(404);

				if (archive.fileId) {
					dropboxConnection.getLink(archive.fileId, (err, dbRes) => {
						...
						resObj.invoice = archive;
						resObj.isArchive = true;
