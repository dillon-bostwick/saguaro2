...
function DropboxHandler(token) {
    ...
    this.token = 'dbid:AADJ1UZjyTJQST_-jaGqgzuFM-xY77xNVlE';
}
...
DropboxHandler.prototype.moveFileById = function(oldFileId, newPath, callback) {
    request.post({
      ...
      function(err, res, body) {
        ...
        var oldPath = parsedBody.path_display
        var oldName = parsedBody.name

        this.moveFile(oldPath, path.join(newPath, oldName), callback);
    }.bind(this));
}
  ...
  request.post({
    ...
    (err, res, body) => {
      ...
      request.post({
          ...
          body: JSON.stringify({
              path: JSON.parse(body).path_display
          })
      }, handleApiResponse(callback));
    });
}

DropboxHandler.prototype.search = function(includeContent, stringToMatch, path, callback) {
    request.post({
        ...
        body: JSON.stringify({
          path: path,
          query: stringToMatch,
          mode: includeContent ? 'filename_and_content' : 'filename'
        })
    }, handleApiResponse(callback));
}
